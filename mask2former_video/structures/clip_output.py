...
class Videos:
    ...
    def __init__(self, num_frames, video_length, num_classes, image_size, device, data_type, num_max_inst=50):
        ...
        self.saved_logits = torch.zeros((video_length, num_max_inst, self.video_length, *image_size), dtype=torch.float,
                                        device=device)
        self.saved_masks = torch.zeros((video_length, num_max_inst, self.video_length, *image_size), dtype=torch.float,
                                       device=device)
    ...
    def get_siou(self, input_masks, saved_masks, saved_valid):
        ...
        print(saved_masks.shape, input_masks.shape)
        ...
        saved_valid = saved_valid[:, :, None, :, None]  # C, N_s, 1, T,
    ...
    def update(self, input_clip):
        ...
        for is_above, r, c in zip(above_thres[row_idx, col_idx], row_idx, col_idx):
            ...
            self.saved_logits[self.num_clip, r, input_clip.frame_idx] = input_clip.mask_logits[c].float()
            self.saved_masks[self.num_clip, r, input_clip.frame_idx] = input_clip.mask_probs[c].float()
        ...
        self.saved_logits[self.num_clip, self.num_inst:self.num_inst + len(left_idx), input_clip.frame_idx] \
            = input_clip.mask_logits[left_idx].float()
        self.saved_masks[self.num_clip, self.num_inst:self.num_inst + len(left_idx), input_clip.frame_idx] \
            = input_clip.mask_probs[left_idx].float()
        self.saved_valid[self.num_clip, self.num_inst:self.num_inst + len(left_idx), input_clip.frame_idx] \
            = True
    ...
    def get_result(self, image_size):
        ...
        _mask_logits = retry_if_cuda_oom(F.interpolate)(
